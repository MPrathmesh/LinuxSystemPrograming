...
void * ThreadProc(void *ptr) //ThreadProc mhnj thread procedure hai just apn apl nav del hai nav windows programming madhe vaperto
{
    ...
    pthread_exit(NULL); //yach use apn apla thread thambvto jeva tech kam complete hot teva
}
...
int main()
{
    ...
    pthread_exit(NULL); //hai apla main thread la band karto jeva tech kam purn hot teva
